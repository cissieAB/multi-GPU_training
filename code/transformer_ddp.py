            ...
            for epoch in range(num_epochs):
                ...
                for batch in train_dataloader:
                    ...
                    for i in range(num_iters):
                        if 40<i<=num_iters-1:  # <==== Updated to 46 because large iters caused stack overflow when tracing 
                            eg = ExecutionTraceObserver()
                            eg.register_callback("./graph_"+name.replace("/","-")+"-"+hostname+"-rank"+str(rank)+"-iter"+str(i)+".json")  # <==== TODO: add host+rank name to guarentee each process has its own trace
                    ...
                    print(f"[{hostname}] Rank {rank}, Local Rank {local_rank}: avg Total time2:", total_time2/10)  # NOTE: Also changed the dividing numbers
        except Exception as e:
